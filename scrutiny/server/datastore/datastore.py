...
class Datastore:
    ...
    def __init__(self) -> None:
        ...
        for watchable_type in WatchableType.all():
            ...
            self._display_path_to_templated_entries_map[watchable_type] = {}
    ...
    def clear(self, watchable_type: Optional[WatchableType] = None) -> None:
        ...
        for type_to_clear in type_to_clear_list:
            ...
            self._display_path_to_templated_entries_map[type_to_clear].clear()

        if WatchableType.Variable in type_to_clear_list:
    ...
    def remove_entry(self, entry_or_entryid: Union[DatastoreEntry, str]) -> None:
        ...
        with tools.SuppressException(KeyError):
            del self._display_path_to_templated_entries_map[entry.get_type()][entry.get_display_path()]

        with tools.SuppressException(KeyError):
            del self._displaypath2idmap[entry.get_type()][entry.get_display_path()]
    ...
    def get_entry_by_display_path(self, display_path: str) -> DatastoreEntry:
        ...
        if parsed_path.has_array_information():
            ...
            if factory_path in self._var_factories:
                ...
                self._display_path_to_templated_entries_map[new_entry.get_type()][display_path] = new_entry
    ...
    def stop_watching(self, entry_or_entryid: Union[DatastoreEntry, str], watcher: str) -> None:
        ...
        if entry.display_path in self._display_path_to_templated_entries_map[entry.get_type()]:
    ...
    def get_all_entries(self, watchable_type: Optional[WatchableType] = None) -> Generator[DatastoreEntry, None, None]:
        ...
        typelist = WatchableType.all() if watchable_type is None else [watchable_type]
        for thetype in typelist:
            for entry_id in self._entries[thetype]:
                yield self._entries[thetype][entry_id]
