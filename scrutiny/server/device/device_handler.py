...
class DeviceHandler:
    ...
    """The configuration coming from the user"""
    datastore: Datastore
    """A reference to the main Datastore"""
    dispatcher: RequestDispatcher
    """The arbiter that receive all request to be sent and decides which one to send (uses a priority queue)"""
    device_searcher: DeviceSearcher
    """Component of the DeviceHandler that search for a new device on the communication link"""
    session_initializer: SessionInitializer
    """Component of the DeviceHandler that try to establish a connection to a found device"""
    heartbeat_generator: HeartbeatGenerator
    """Component of the DeviceHandler that periodically enqueue heartbeat request to keep a session active"""
    memory_reader: MemoryReader
    """Component of the DeviceHandler that polls the device memory to keep the datastore watched entries in sync with the device"""
    memory_writer: MemoryWriter
    """Component of the DeviceHandler that fulfill writes requests on datastore entries"""
    info_poller: InfoPoller
    """Component of the DeviceHandler that will gather all device parameters after a connection is established"""
    datalogging_poller: DataloggingPoller
    """Component of the DeviceHandler that will interact with the device datalogging feature"""
    comm_handler: CommHandler
    """Layer that handle the communication with the device. Converts Requests to bytes and bytes to response. Also tells if a request timed out"""
    protocol: Protocol
    """The communication protocol. Encode and decodes request/response payload to meaning ful data"""
    device_info: Optional[DeviceInfo]
    """All the information about the device gathered by the InfoPoller"""
    comm_broken: bool
    """Flag indicating that we lost connection with the device. Resets the communication and the state machine."""
    device_id: Optional[str]
    """Firmware ID of the device on which we are connected."""
    operating_mode: "DeviceHandler.OperatingMode"
    """Operating mode - Normal or special modes for unit testing"""
    connected: bool
    """True when a connection to a device has been made"""
    fsm_state: "DeviceHandler.FsmState"
    """The internal state machine state"""
    last_fsm_state: "DeviceHandler.FsmState"
    """The state machine state at the previous execution cycle"""
    active_request_record: Optional[RequestRecord]
    """Request to the device on which we are waiting for"""
    device_session_id: Optional[int]
    """The session ID given by the device upon connection"""
    disconnection_requested: bool
    """The external world requested that we disconnect from the device"""
    disconnect_callback: Optional[DisconnectCallback]
    """Callback called upon disconnection"""
    disconnect_complete: bool
    """Flags indicating that a disconnect request has been completed"""
    comm_broken_count: int
    """Counter keeping track of how many time we had communication issues"""
    fully_connected_ready: bool
    """Indicates that the DeviceHandler is connected to a device and the initialization phase is correctly completed"""
    wait_clean_state_timestamp: float
    """Timestamp to detect subcomponents that might be stuck"""
    server_session_id: Optional[str]
    """A session id generated by the DeviceHandler upon connection"""
    device_state_changed_callbacks: List[DeviceStateChangedCallback]
    """Callback called when the state machine changes state"""
    expect_no_timeout: bool
    """Flag that makes communication timeout a critical error. Used for unit testing."""
    device_session_count: int
    ...
    def __init__(self,
        ...
        try:
            validation.assert_dict_key(self.config, 'default_protocol_version', str)
            validation.assert_dict_key(self.config, 'response_timeout', (int, float))
            validation.assert_dict_key(self.config, 'heartbeat_timeout', (int, float))
            validation.assert_dict_key(self.config, 'default_address_size', int)
            validation.assert_dict_key(self.config, 'max_request_size', int)
            validation.assert_dict_key(self.config, 'max_response_size', int)
            validation.assert_dict_key(self.config, 'max_bitrate_bps', (int, float))
        except Exception as e:
            raise ValueError(f"Error in device handler configuration. {e}") from e
