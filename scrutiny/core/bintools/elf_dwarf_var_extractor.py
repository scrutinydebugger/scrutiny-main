...
class VarPath:
    ...
    """The segments of the path. Each element contains a string and array information that goes with that segment"""
...
class CuName:
    ...
    _fullpath: str
    """the full path of the compile unit gotten from the dwarf symbol"""
    _filename: str
    """The basename of ``_fullpath``"""
    _display_name: str
    """The name displayed to the user. Subsection of the fullpath"""
    _segments: List[str]
    """The fullpath broken down in string segments"""
    _numbered_name: Optional[str]
    """A numbered name autogenerated to overcome name collisions"""
...
class ParseErrors:
    ...
    We keep track of all the error we find so that we can fail unit tests without stopping the parsing"""
    __slots__ = ('_exceptions', )

    _exceptions: List[Exception]
    """List of exception registered"""

    def __init__(self) -> None:
        self._exceptions = []

    def register_error(self, e: Exception) -> None:
        """Add a catched exception"""
        self._exceptions.append(e)

    def total_count(self, exclude: Optional[List[Type[Exception]]] = None) -> int:
        """Return the number of exception that are not excluded by ``exclude``"""
        if exclude is None:
    ...
    def iter_exc(self, exclude: Optional[List[Type[Exception]]] = None) -> Generator[Exception, None, None]:
        """Iterate every exception registered with an exclude list"""
...
class ElfDwarfVarExtractor:
    ...
    """Global flag disabling or enabling pointer dereferencing. This may come from user with a CLI parameter"""
    ...
    def _get_die_name(self,
        ...
        :param raise_if_none: Raise an error if no name is available and no default name is possible
...
# region DIE specific
    ...
    def _die_process_ptr_type(self, die: DIE) -> None:
        """Process DIE of type Pointer. Register the type in the global index and maps it to a known type."""
        self._log_debug_process_die(die)
        if die not in self._die2vartype_map:
    ...
    def _die_process_enum(self, die: DIE) -> None:
        """Process a DIE of type Enum. Get a full definition and register an EmbeddedEnum structure in a global cache"""
        self._log_debug_process_die(die)
...
# endregions
    ...
    def _get_pointee_type_of_var(self, ptr_die: DIE) -> PointeeTypeDescriptor:
        ...
        VOID_PTR = PointeeTypeDescriptor(TypeOfVar.Void, None, None)
