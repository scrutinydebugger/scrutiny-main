...
class MathTreeModel(BaseTreeModel):
    ...
    def insert_math_element(self, math_element: GUIMathWatchable) -> None:
        ...
        for name in sorted(name_fqn_map.keys()):
            fqn_name = name_fqn_map[name]
            if fqn_name is None:
                raise ValueError(f"No watchable element assigned to variable {name}")

            var_item = WatchableStandardItem(FQN.parse(fqn_name.fqn).node_type, name, "TODO")
