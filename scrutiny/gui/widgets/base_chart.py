...
_PENSTYLE_SERIALIZABLE_MAP: Dict[Qt.PenStyle, str] = {
    Qt.PenStyle.SolidLine: 'solid',
    Qt.PenStyle.DashLine: 'dash',
    Qt.PenStyle.DotLine: 'dot'
}

_PENSTYLE_SERIALIZABLE_MAP_REVERSED = {v: k for k, v in _PENSTYLE_SERIALIZABLE_MAP.items()}


@dataclass(frozen=True, slots=True)
class SingleTypeGridConfiguration:
    class SerializableDict(TypedDict):
        visible: bool
        tick_count: int
        line_style: str
        line_width: int
    ...
    def to_serializable_dict(self) -> SerializableDict:
        line_style_str = _PENSTYLE_SERIALIZABLE_MAP[Qt.PenStyle.SolidLine]
        if self.line_style in _PENSTYLE_SERIALIZABLE_MAP:
            line_style_str = _PENSTYLE_SERIALIZABLE_MAP[self.line_style]

        return {
            'visible': self.visible,
            'tick_count': self.tick_count,
            'line_style': line_style_str,
            'line_width': self.line_width
        }

    @classmethod
    def from_serializable_dict(cls, d: SerializableDict, default: Self) -> Self:
        style_str = d.get('line_style', '')
        return cls(
            visible=bool(d.get('visible', default.visible)),
            tick_count=int(d.get('tick_count', default.tick_count)),
            color=QColor(default.color),    # Depends on the theme. Do not load from serializable data
            line_style=_PENSTYLE_SERIALIZABLE_MAP_REVERSED.get(style_str, default.line_style),
            line_width=int(d.get('line_width', default.line_width))
        )
...
class GridConfiguration:
    class SerializableDict(TypedDict):
        major: SingleTypeGridConfiguration.SerializableDict
        minor: SingleTypeGridConfiguration.SerializableDict

    major: SingleTypeGridConfiguration
    minor: SingleTypeGridConfiguration

    @classmethod
    def make_default(cls) -> Self:
        chart = ScrutinyChart()
        axis = ScrutinyValueAxis()
        chart.addAxis(axis, Qt.AlignmentFlag.AlignLeft)

        major_pen = axis.gridLinePen()
        minor_pen = axis.minorGridLinePen()

        return cls(
            major=SingleTypeGridConfiguration(
                visible=axis.isGridLineVisible() if axis.tickCount() > 0 else False,
                color=major_pen.color(),
                line_style=major_pen.style(),
                line_width=major_pen.width(),
                tick_count=axis.tickCount()
            ),
            minor=SingleTypeGridConfiguration(
                visible=axis.isMinorGridLineVisible() if axis.minorTickCount() > 0 else False,
                color=minor_pen.color(),
                line_style=minor_pen.style(),
                line_width=minor_pen.width(),
                tick_count=axis.minorTickCount()
            )
        )

    @classmethod
    def from_serializable_dict(cls, d: SerializableDict) -> Self:
        default_config = cls.make_default()
        return cls(
            major=SingleTypeGridConfiguration.from_serializable_dict(d.get('major', {}), default_config.major),
            minor=SingleTypeGridConfiguration.from_serializable_dict(d.get('minor', {}), default_config.minor)
        )

    def to_serializable_dict(self) -> SerializableDict:
        return {
            'major': self.major.to_serializable_dict(),
            'minor': self.minor.to_serializable_dict()
        }
...
class ScrutinyValueAxis(QValueAxis):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.setMinorGridLineColor(palette.window().color())
...
class ScrutinyValueAxisWithMinMax(ScrutinyValueAxis):
    ...
    def apply_zoombox_y(self, zoombox: QRectF) -> None:
        ...
        Y limits of the new zoom. Values are normalized to p.u.
...
class ScrutinyChart(QChart):
    ...
    _grid_config: Optional[GridConfiguration]
    ...
    def addAxis(self, axis: QAbstractAxis, alignment: Qt.AlignmentFlag) -> None:
        super().addAxis(axis, alignment)
        if self._grid_config is not None:
            self.apply_grid_config(self._grid_config)
    ...
    def apply_grid_config(self, config: GridConfiguration) -> None:
        for axis in self.axes():
            assert isinstance(axis, QValueAxis)

            major_pen = axis.gridLinePen()
            minor_pen = axis.minorGridLinePen()

            major_pen.setWidth(config.major.line_width)
            major_pen.setStyle(config.major.line_style)
            major_pen.setColor(config.major.color)

            minor_pen.setWidth(config.minor.line_width)
            minor_pen.setStyle(config.minor.line_style)
            minor_pen.setColor(config.minor.color)

            axis.setGridLineVisible(config.major.visible)
            axis.setTickCount(config.major.tick_count)
            axis.setGridLinePen(major_pen)

            axis.setMinorGridLineVisible(config.minor.visible)
            axis.setMinorTickCount(config.minor.tick_count)
            axis.setMinorGridLinePen(minor_pen)
        self.update()

    def set_grid_config(self, config: GridConfiguration) -> None:
        self._grid_config = config
        self.apply_grid_config(config)

    def get_grid_config(self) -> GridConfiguration:
        axes = list(self.axes())
        if len(axes) == 0:
            return GridConfiguration.make_default()

        axis = cast(ScrutinyValueAxis, axes[0])
        major_pen = axis.gridLinePen()
        minor_pen = axis.minorGridLinePen()

        return GridConfiguration(
            major=SingleTypeGridConfiguration(
                visible=axis.isGridLineVisible(),
                tick_count=axis.tickCount(),
            ...
            minor=SingleTypeGridConfiguration(
                visible=axis.isMinorGridLineVisible(),
                tick_count=axis.minorTickCount(),
                color=axis.minorGridLineColor(),
                line_style=minor_pen.style(),
                line_width=minor_pen.width(),
            )
        )
...
class ScrutinyChartView(QChartView):
    ...
    def cursor1_enabled(self) -> bool:
        """Tells if the chart cursor is enabled"""
        return self._chart_cursor_x1.is_enabled()

    def cursor2_enabled(self) -> bool:
        """Tells if the chart cursor is enabled"""
    ...
    def enable_cursor1(self) -> None:
        ...
        self._chart_cursor_x1.enable()
        self._chart_cursor_x1.set_xval(xaxis.min() + range / 2)
        ...
        self._chart_cursor_x1.clear_markers()
    ...
    def enable_cursor2(self) -> None:
        ...
        self._chart_cursor_x2.clear_markers()
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        if self._chart_cursor_x1.is_in_drag_zone(event.pos().toPointF()):
            event.accept()
            self._chart_cursor_x1.start_drag()
        elif self._chart_cursor_x2.is_in_drag_zone(event.pos().toPointF()):
            event.accept()
            self._chart_cursor_x2.start_drag()
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self._chart_cursor_x1.is_dragging():
            ...
            dragged_cursor = self._chart_cursor_x2

        # The user is dragging the chart cursor (red vertical line)
        if dragged_cursor is not None:
            event.accept()
            ...
            dragged_cursor.clear_markers()
            if xval is not None:
                snap_xval, point_series_pairs = self._get_closest_x_to_snap_to(xval)
                if snap_xval is not None:
                    dragged_cursor.set_xval(snap_xval)
                    dragged_cursor.set_markers(point_series_pairs)

            self._invalidate_forground()
            self.update()

        elif self._chart_cursor_x1.is_in_drag_zone(event.pos().toPointF()) or self._chart_cursor_x2.is_in_drag_zone(event.pos().toPointF()):
            mouse_cursor = Qt.CursorShape.SizeHorCursor
        elif self._interaction_mode == self.InteractionMode.DRAG and self._drag_allowed:
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        if self._chart_cursor_x1.is_dragging():
            self._chart_cursor_x1.stop_drag()
            require_foreground_redraw = True
        if self._chart_cursor_x2.is_dragging():
            self._chart_cursor_x2.stop_drag()
            require_foreground_redraw = True
    ...
    def _draw_cursor(self, cursor: ChartCursor, painter: QPainter, color: QColor) -> None:
        ...
        if cursor_xpos_mapped_to_chart is not None:
            ...
            plotarea_width = max(chart.plotArea().width(), 1)
            cursor_val_width = (xaxis.max() - xaxis.min()) / plotarea_width
            cursor_xval = cursor.xval()
            painter.setPen(self._text_color)

            for pair in cursor.get_markers_vals():
...
class ScrutinyChartToolBar(QGraphicsItem):
    ...
    def disable_chart_cursors(self) -> None:
        self._chartview.disable_cursors()
        self.update_buttons_from_state()

    def disable_chart_cursor2(self) -> None:
