...
class SFDUploadRequest(PendingRequest):
    ...
    _firmware_id: str
    """Firmware ID of the SFD uploaded"""
    _token: str
    """A token generated by the client that we will feed back for cancelling"""
    _overwritten:Optional[bool]
    """Indicate if the upload overwrote an existing SFD. Has a value only on successful, complete upload"""

    def __init__(self, client: "ScrutinyClient", firmware_id: str, unique_token:str) -> None:
        super().__init__(client)
    ...
    def _set_overwritten(self, val:bool) -> None:
        self._overwritten = val

    @property
    def overwritten(self) -> bool:
        """Tells if the upload overwrote an existing SFD on the server. This value is only available when :attr:`is_success<is_success>` is ``True``"""
        if not self.is_success or self._overwritten is None:
            raise sdk.exceptions.InvalidValueError("Request has not completed successfully. Value not available")
        return self._overwritten
    
    def cancel(self) -> None:
        self._client._cancel_upload_sfd_request(self._token)
...
class ScrutinyClient:
    ...
    def _cancel_download_sfd_request(self, reqid: int) -> None:
        with self._main_lock:
            if reqid not in self._pending_sfd_download_requests:
                raise sdk.exceptions.OperationFailure(f"No download request identified by request ID : {reqid}")
            self._pending_sfd_download_requests[reqid]._mark_complete(success=False, failure_reason="Cancelled by user")
    ...
    def upload_sfd(self, filepath: Union[str, Path], timeout: Optional[float] = None) -> SFDUploadRequest:
        """Upload a Scrutiny Firmware Description file to the server. Blocks until completion. 
        ...
        @dataclass
        class Container:
            obj: Optional[api_parser.UploadSFDConfirmation]
