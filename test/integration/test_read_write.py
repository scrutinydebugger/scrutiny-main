...
class TestReadWrite(ScrutinyIntegrationTestWithTestSFD1):
    ...
    def test_write_read_ptr(self):
        entries_to_subscribe: List[DatastoreEntry] = [
            self.entry_ptr1_32bits,
            self.entry_ptr1_pointee_offset0_fp32,
            self.entry_ptr1_pointee_offset4_u16,
            self.entry_float32,
            self.entry_u16]
        self.init_device_memory([self.entry_ptr1_32bits])
        # Init pointed memory. Pointers goes up to pointed address+6. we allow to move the pointer from -6 to +6 without triggering a bad memory access
        self.emulated_device.write_memory(self.entry_float32.get_address() - 6, b"\x00" * 6 * 2)
        self.emulated_device.write_memory(self.entry_u16.get_address() - 6, b"\x00" * 6 * 2)

        subscribe_cmd = {
            'cmd': API.Command.Client2Api.SUBSCRIBE_WATCHABLE,
            'watchables': [entry.get_display_path() for entry in entries_to_subscribe]
        }
        self.send_request(subscribe_cmd)
        response = self.wait_and_load_response()
        self.assert_no_error(response)
        ...
        self.assertEqual(struct.unpack('<f', data_fp32)[0], d2f(3.1415926))
        self.process_watchable_update(len(entries_to_subscribe) * 2)
        self.assert_value_received(self.entry_float32, d2f(3.1415926))
        self.assert_value_received(self.entry_ptr1_pointee_offset0_fp32, d2f(3.1415926))

        # Write ptr for u16 read. Pointed item uses offset 4, so we move the pointer with -4
        write_req = {
            'cmd': API.Command.Client2Api.WRITE_WATCHABLE,
            'updates': [dict(watchable=self.entry_ptr1_32bits.get_id(), value=self.entry_u16.get_address() - 4, batch_index=0)]
        }
        ...
        # Write u16 through pointee
        write_req = {
            'cmd': API.Command.Client2Api.WRITE_WATCHABLE,
            'updates': [dict(watchable=self.entry_ptr1_pointee_offset4_u16.get_id(), value=d2f(0x1234), batch_index=0)]
        }
        ...
        self.assert_value_received(self.entry_ptr1_pointee_offset4_u16, 0x1234)
