...
#        A test suite to test the scrutiny.core.MathWatchable
#
...
class TestMathWatchable(ScrutinyUnitTest):
    def test_no_vars(self):
        watchable = MathWatchable('test', '1+2+3')
    ...
    def test_with_single_var(self):
        watchable = MathWatchable('test', '1+2+a')
        self.assertTrue(watchable.is_valid())
    ...
    def test_with_many_vars(self):
        watchable = MathWatchable('test', '1+2+a*b+abs(-x)')
        self.assertTrue(watchable.is_valid())
    ...
    def test_duplicate_fqn_is_fine(self):
        watchable = MathWatchable('test', '1+2+a1*a2+abs(-x)')
        self.assertTrue(watchable.is_valid())
    ...
    def test_cannot_bind_inexistant_fqn(self):
        watchable = MathWatchable('test', 'a+b')
        self.assertTrue(watchable.is_valid())
    ...
    def test_rebind_is_allowed(self):
        watchable = MathWatchable('test', '1+a')
        self.assertTrue(watchable.is_valid())
        watchable.bind_watchable("a", 'var:/a/a/a')
    ...
    def test_variable_case_sensitive(self):
        watchable = MathWatchable('test', '1+hElLo_World+aaa+AAA')
        self.assertTrue(watchable.is_valid())
        vars = watchable.get_vars()
        self.assertEqual(len(vars), 3)
    ...
    def test_func_are_case_insensitive(self):
        watchable = MathWatchable("test", "1+sIn(0.5) - COS(2)")
        val = watchable.eval()
    ...
    def test_copy(self):
        watchable = MathWatchable("test", "1+sin(x)-cos(y)")
